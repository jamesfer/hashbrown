...
use crate::new_map_2::iterable_bit_mask::{IterableBitMask, IterableBitMaskIntrinsics, IterableBitMaskIntrinsics16x4, IterableBitMaskT};
...
#[cfg(test)]
mod utils_tests {
    use crate::new_map_2::fixed_table::{get_positional_overflow_bit_or_mask, overwrite_overflow_bit, OVERFLOW_BIT_MASK};
...
pub struct FixedTable<V> {
    size: usize,
    chunks: UnsafeCell<Box<[Chunk<V>]>>,
}

unsafe impl <T: Send> Send for FixedTable<T> {}
unsafe impl <T: Sync> Sync for FixedTable<T> {}

impl <V> FixedTable<V>
where V: Default + Copy + AsAtomic + PartialEq + 'static
{
    pub fn new_with_capacity(capacity: usize) -> Self {
        let chunk_count = (capacity.next_multiple_of(16) / 16).next_power_of_two();
        Self {
            size: chunk_count * 16,
            chunks: UnsafeCell::new((0..chunk_count).map(|_| Chunk::new()).collect()),
        }
    }
    ...
    pub fn to_read_only(self) -> ReadOnlyTable<V> {
        ReadOnlyTable::new(self.chunks)
    }
    ...
    pub fn entries(&self) -> impl Iterator<Item=(u64, V)> {
        unsafe { &*self.chunks.get() }.iter()
            .map(|chunk| {
                ...
                chunk.tags.iter()
                    .zip(chunk.data[0..15].iter())
                    .filter(|(_, (hash, _))| *hash & HASH_OCCUPIED_BIT != 0)
                    .map(|(tag, (hash, value))| ((*hash & IGNORE_TOP_8_MASK) | ((*tag as u64) << 56), value.clone()))
            })
            .flatten()
    }

    pub fn write(&self, hash: u64, value: V) -> Result<Option<V>, ()> {
        let mut loaded_tags = [0u8; 16];
        let mut match_output = [0u8; 16];
    ...
    pub fn write_using_buffers(
        ...
        hash: u64,
        value: V,
        loaded_tags: &mut [u8; 16],
        match_output: &mut [u8; 16],
    ) -> Result<Option<V>, ()> {
        ...
        let search_tag = top_8_bits;
        let stored_hash = hash & IGNORE_TOP_8_MASK | HASH_OCCUPIED_BIT;
        ...
        while attempts < chunks.len() {
            ...
            let mut bit_mask = IterableBitMask::new(match_output.clone());
            while let Some(position) = bit_mask.next_set_index() {
                ...
                let masked_item_hash = item_hash & IGNORE_TOP_7_MASK;
            ...
            while let Some(position) = bit_mask.next_set_index() {
                ...
                let masked_stored_hash = stored_hash | HASH_OCCUPIED_BIT;
            ...
            let data = &mut chunk.data[15];
            let hash_ref = &mut data.0;
            let hash_ptr = core::ptr::from_mut(hash_ref);
            let atomic_hash = unsafe { AtomicU64::from_ptr(hash_ptr) };
...
pub struct ReadOnlyTable<V> {
    chunk_mask: usize,
    chunks: UnsafeCell<Box<[Chunk<V>]>>
}

unsafe impl <T: Send> Send for ReadOnlyTable<T> {}
unsafe impl <T: Sync> Sync for ReadOnlyTable<T> {}

impl <V> ReadOnlyTable<V> {
    fn new(chunks: UnsafeCell<Box<[Chunk<V>]>>) -> Self {
        let chunk_mask = {
    ...
    pub fn get(&self, hash: u64) -> Option<&V> {
        ...
        let top_8_bits = (hash >> 56) as u8;
        let search_tag = top_8_bits;
        // The hash 0 is reserved to represent an empty cell
        let stored_hash = hash & IGNORE_TOP_8_MASK | HASH_OCCUPIED_BIT;
        ...
        loop {
            ...
            if let Some(value) = Self::check_match_tags(stored_hash, search_tag, &chunk) {
                return Some(value);
            }

            // If the overflow bit isn't set we can short circuit out of the search loop early
            let overflow_bit = chunk.data[15].0 & OVERFLOW_BIT_MASK;
            if overflow_bit == 0 {
                return None;
            }
    ...
    #[inline]
    fn check_match_tags<'a>(stored_hash: u64, search_tag: u8, chunk: &'a Chunk<V>) -> Option<&'a V> {
        for position in Self::iter_matched_tags(search_tag, chunk) {
            ...
            if let Some(value) = Self::check_item_matches_stored_hash(stored_hash, &item) {
                return Some(value);
            }
        }
        None
    ...
    #[inline]
    fn check_item_matches_stored_hash(stored_hash: u64, item: &(u64, V)) -> Option<&V> {
        if item.0 & IGNORE_TOP_7_MASK == stored_hash {
            return Some(&item.1);
        }
        None
    }
    ...
    pub fn measure_get_stats(&self, hash: u64) -> (Option<&V>, ReadStats) {
        ...
        while attempts < chunks.len() {
            ...
            for position in Self::iter_matched_tags(search_tag, chunk) {
                ...
                if item.0 & IGNORE_TOP_7_MASK == stored_hash {
                    return (Some(&item.1), ReadStats { chunks_accessed: attempts, tag_false_positives});
                } else {
            ...
            let overflow_bit = chunk.data[15].0 & OVERFLOW_BIT_MASK;
            if overflow_bit == 0 {
                return (None, ReadStats { chunks_accessed: attempts, tag_false_positives });
            }
    ...
    #[inline]
    fn iter_matched_tags(search_tag: u8, chunk: &Chunk<V>) -> IterableBitMaskIntrinsics16x4 {
        #[cfg(target_arch = "aarch64")]
        {
            use crate::new_map_2::intrinsics::u8x16_compare_eq_aarch64_shrink;

            let value = u8x16_compare_eq_aarch64_shrink(search_tag, &chunk.tags);
            IterableBitMaskIntrinsics16x4::new(value)
        }
...
mod tests {
    ...
    fn create_empty_table() {
        let table = FixedTable::<usize>::new_with_capacity(20);
    ...
    fn can_read_from_a_full_table() {
        let table = FixedTable::<usize>::new_with_capacity(64);
    ...
    fn can_store_all_special_values() {
        ...
        // colliding
        let table = FixedTable::<usize>::new_with_capacity(16);
    ...
    async fn multithreaded_writing_test() {
        ...
        let table = Arc::new(FixedTable::<usize>::new_with_capacity(batch_size * thread_count));
    ...
    async fn supports_writing_zero_hash_in_parallel() {
        ...
        let tables = (0..table_count).map(|_| FixedTable::<usize>::new_with_capacity(thread_count)).collect::<Vec<_>>();
        ...
        let tables = Arc::new((0..table_count).map(|_| FixedTable::<usize>::new_with_capacity(thread_count)).collect::<Vec<_>>());
